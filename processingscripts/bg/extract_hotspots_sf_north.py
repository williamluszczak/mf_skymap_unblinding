...
for j in range(seednum, seednum+1):
    f = '/data/user/wluszczak/multiflare_csky/gridmaps/%s/pmap_north_%s.npy'%(j, j)
    print(f)
    ...
    np.save('/data/user/wluszczak/multiflare_csky/gridmaps/%s/north_hotspots_sf_%s.npy'%(j, j), hotspots)
