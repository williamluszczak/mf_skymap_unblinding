...
inj_evts = []
for decband in range(0,60):
    print(decband)
    data = np.load(inputdir+'bgmap_north_%s_%s.npz'%(seednum, decband), allow_pickle=True, encoding='bytes')
    tsdata = data['arr_0']
...
np.save('/data/user/wluszczak/multiflare_csky/gridmaps/%s/combinedmap_north_%s.npy'%(seednum, seednum), pixarr)
...
ax.grid(**kw)
cb.set_label(r'TS')
plt.savefig('/data/user/wluszczak/multiflare_csky/gridmaps/%s/skymap_north_%s.png'%(seednum, seednum), dpi=500)
