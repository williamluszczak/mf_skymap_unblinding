...
lowerdec = np.arcsin(np.sin(np.radians(-5.))+(1.0834/60.)*(decband))
upperdec = np.arcsin(np.sin(np.radians(-5.))+(1.0834/60.)*(decband+1))
...
#    firefile = np.loadtxt('/data/user/wluszczak/FIRESONG-base/Results/d%s/d%s_%s.out'%(rho, rho, inputseed))
    ...
    firesong_srcfluxes = firesong_srcfluxes[np.where((firesong_decs>lowerdec-np.radians(2.)) & (firesong_decs<upperdec+np.radians(2.)))]
    firesong_ras = firesong_ras[np.where((firesong_decs>lowerdec-np.radians(2.)) & (firesong_decs<upperdec+np.radians(2.)))]
    firesong_decs = firesong_decs[np.where((firesong_decs>lowerdec-np.radians(2.)) & (firesong_decs<upperdec+np.radians(2.)))]
    ...
    for srcind in range(0,len(firesong_srcfluxes)):
        ...
        if howmanyflares == 0 and srcflux>0:
            howmanyflares = 1
...
t1 = time.time()
msstr = cy.conf.get_sky_scan_trial_runner(ana=ana, multiflare=True, nside=256, threshold=SoBcut, max_dt=400., flux=hyp.PowerLawFlux(2), min_dec=lowerdec, max_dec=upperdec)
...
np.savez('data/bgmap_north_%s_%s.npz'%(inputseed,decband), tsarr, flarecurves)
